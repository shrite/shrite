...
class EDD_Theme_Updater_Admin {
	...
	function updater() {
		...
		if ( !class_exists( 'EDD_Theme_Updater' ) ) {
			...
			require_once ( dirname( __FILE__ ) . '/theme-updater-class.php' );
		}
	 ...
	 */
	 function get_api_response( $api_params ) {
		...
		if ( is_wp_error( $response ) ) {
			wp_die( $response->get_error_message(), __( 'Error' , 'beonepage' ) . $response->get_error_code() );
		}
	...
	function activate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = __( 'An error occurred, please try again.', 'beonepage' );
			}
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'expired' :
						...
						$message = sprintf(
							__( 'Your license key expired on %s.', 'beonepage' ),
					...
					case 'revoked' :
						...
						$message = __( 'Your license key has been disabled.', 'beonepage' );
						break;

					case 'missing' :

						$message = __( 'Invalid license.', 'beonepage' );
						break;

					case 'invalid' :
					case 'site_inactive' :

						$message = __( 'Your license is not active for this URL.', 'beonepage' );
						break;

					case 'item_name_mismatch' :

						$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'beonepage' ), $args['name'] );
						break;

					case 'no_activations_left':

						$message = __( 'Your license key has reached its activation limit.', 'beonepage' );
						break;
