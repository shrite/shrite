...
<tr class="shipping">
	<th><?php
		if ( $show_package_details ) {
			printf( __( 'Shipping #%d', 'beonepage' ), $index + 1 );
		} else {
			_e( 'Shipping', 'beonepage' );
		}
	?></th>
	<td>
		...
		<?php elseif ( ( WC()->countries->get_states( WC()->customer->get_shipping_country() ) && ! WC()->customer->get_shipping_state() ) || ! WC()->customer->get_shipping_postcode() ) : ?>
			...
			<?php if ( is_cart() && get_option( 'woocommerce_enable_shipping_calc' ) === 'yes' ) : ?>
				...
				<p><?php _e( 'Please use the shipping calculator to see available shipping methods.', 'beonepage' ); ?></p>

			<?php elseif ( is_cart() ) : ?>

				<p><?php _e( 'Please continue to the checkout and enter your full address to see if there are any available shipping methods.', 'beonepage' ); ?></p>

			<?php else : ?>

				<p><?php _e( 'Please fill in your details to see available shipping methods.', 'beonepage' ); ?></p>
		...
		<?php else : ?>
			...
			<?php if ( is_cart() ) : ?>
				...
				<?php echo apply_filters( 'woocommerce_cart_no_shipping_available_html',
					'<p>' . __( 'There are no shipping methods available. Please double check your address, or contact us if you need any help.', 'beonepage' ) . '</p>'
				); ?>
		...
		<?php if (isset($show_package_details)) : ?>
			<?php
				...
				echo '<p class="woocommerce-shipping-contents"><small>' . __( 'Shipping', 'beonepage' ) . ': ' . implode( ', ', $product_names ) . '</small></p>';
			?>
		<?php endif; ?>
